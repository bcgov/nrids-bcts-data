...
SELECT                                                           --DISTINCT
	 LRM_REPLICATION.LICENCE_ALLOCATION.MANU_SEQ_NBR,
	  ...
	  (SELECT ROUND(CAST(COALESCE(SUM(ST_Area(V.SHAPE)) / 10000, 0) AS numeric), 1)
	  ...
	  CASE
		 WHEN (SELECT COUNT (*)
				 FROM LRM_REPLICATION.LICENCE_ALLOCATION
				WHERE LRM_REPLICATION.LICENCE_ALLOCATION.LICN_SEQ_NBR =
						 LRM_REPLICATION.LICENCE.LICN_SEQ_NBR) > 0
		 THEN
			'Y'
		 ELSE
			'N'
 ...
 FROM LRM_REPLICATION.LICENSEE
	  ...
	  JOIN LRM_REPLICATION.LICENCE_ALLOCATION
