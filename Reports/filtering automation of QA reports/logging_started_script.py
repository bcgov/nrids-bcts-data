...
# Filter ‘ISSUED_FISCAL’ for <2004 and delete
...
# Filter ‘HVS_STATUS’ for ‘D, N or A’ and delete 
...
# Condition 1: If logging Completed ‘Done’ with no Logging Started ‘Done’
condition_1 = (
    df_1['HVC_STATUS'] == 'D'
)

# Condition 2: If block waste survey completed with no block Logging Started ‘Done’
condition_2 = (
...
# Condition 3: If cruise base reconciliation indicates licence completion with no block Logging Started ‘Done’
condition_3 = (
    (df_1['CRUISE_BASED_IND'] == 'Y') &
    ((df_1['Sold_Variance_m3'] >= -10) & (df_1['Sold_Variance_m3'] <= 10))
)

# Condition 4: If licence closed with no block Logging Started ‘Done’
condition_4 = (
    df_1['HC_STATUS'] == 'D'
)

# Condition 5: If licence substantially complete with no block Logging Started ‘Done’
condition_5 = (
    df_1['SUBCOMP_STATUS'] == 'D'
)

# Condition 6: If licence expired with no block Logging Completed ‘Done’
condition_6 = (
    (
...
# Condition 7: If block state indicates Logging Started ‘Done’
condition_7 = (
    (df_1['BLOCK_STATE'] == 'HB') |
    (df_1['BLOCK_STATE'] == 'LC') | 
    (df_1['BLOCK_STATE'] == 'RG') | 
...
# Condition 8: If licence State indicates Logging Started ‘Done’
condition_8 = (
...
# Condition 9: If FTA file status indicates block Logging Started ‘Done’
condition_9 = (
...
conditions = [
    (condition_1, "Logging Completed ‘Done’ with no Logging Started ‘Done’"),
    (condition_2, "Block waste survey completed with no block Logging Completed 'Done'"),
    (condition_3, "Cruise base reconciliation indicates licence completion with no block Logging Completed 'Done'"),
    (condition_4, "Licence closed with no block Logging Completed 'Done'"),
    (condition_5, "Licence substantially complete with no block Logging Completed 'Done'"),
    (condition_6, "Licence expired with no block Logging Completed 'Done'"),
    (condition_7, "Block state indicates Logging Completed 'Done'"),
    (condition_8, "Licence State indicates Logging Completed 'Done'"),
    (condition_9, "FTA File Status indicates block Logging Completed 'Done'"),
...
output_name_pattern = f'{output_folder}/Logging_Started_Conformance_Summary_YYYY_MM_DD.xlsx'
    ...
    for sheet_name in selected_df['BUSINESS_AREA_CODE'].unique().tolist() + ['Province']: # Iterate through each regional + province sheet
        ...
        for row_idx, (_, row) in enumerate(region_df.iterrows(), start=1):
            ...
            if (
                ...
                worksheet.write(row_idx, column_to_index['HVC_STATUS'], row['HVC_STATUS'], yellow_format)
