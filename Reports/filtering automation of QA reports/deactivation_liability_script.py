...
try:
    ...
    file_pattern = os.path.join(script_dir, '../data', 'QA_Roads_Deactivation_*_*.xlsx')
    ...
    if not matching_files:
        raise FileNotFoundError(
            ...
            'QA_Roads_Deactivation_YYYY_MM_DD.xlsx'
            """
        )
    ...
    df = pd.read_excel(file_path, sheet_name='qQA_RoadsDeactivation')
...
# Condition 1: If deactivation method is missing
condition_1 = (
    ((df_1['RCLS_ACCOUNTING_TYPE'] == '1 Sale = 5 yrs') | (df_1['RCLS_ACCOUNTING_TYPE'] == 'S.Term = 10 yrs')) & 
    (df_1['DEAC_METHOD_TYPE'].isnull())
)

# Condition 2: If cost for 5 or 10 year roads is missing or incorrect
condition_2 = (
    ((df_1['RCLS_ACCOUNTING_TYPE'] == '1 Sale = 5 yrs') | (df_1['RCLS_ACCOUNTING_TYPE'] == 'S.Term = 10 yrs')) &
    ((df_1['EFFECTIVE_PLANNED_COST'].isnull()) | (df_1['EFFECTIVE_PLANNED_COST'] == 0) | (df_1['EFFECTIVE_PLANNED_COST'] == 1))
)


# Condition 3: If planned deactivation date for 5 or 10 year roads is missing
condition_3 = (
    ((df_1['RCLS_ACCOUNTING_TYPE'] == '1 Sale = 5 yrs') | (df_1['RCLS_ACCOUNTING_TYPE'] == 'S.Term = 10 yrs')) &
    df_1['DEAC_PLANNED_DATE'].isnull()
)

# Condition 4: If cost for 5 year deactivation window is missing or incorrect
condition_4 = (
    (df_1['PLANNED_FISCAL_YEAR'] < (current_fiscal + 5)) &
    ((df_1['EFFECTIVE_PLANNED_COST'].isnull()) | (df_1['EFFECTIVE_PLANNED_COST'] == 0) | (df_1['EFFECTIVE_PLANNED_COST'] == 1))
)

# Condition 5: If planned deactivation from previous fiscal is past due
condition_5 = (df_1['PLANNED_FISCAL_YEAR'] < current_fiscal)

# Condition 6: If planned deactivation is overlapping or duplicated
condition_6 = (df_1['OVER_LAPPING'] == 'Y')

# Make a list for all the conditions and their corresponding column names
conditions = [
    (condition_1, "Missing from liability because of no Deact Method"),
    (condition_2, "Missing or incorrect Cost for 5 or 10 year road"),
    (condition_3, "Missing Planned deactivation date for 5 or 10 year road"),
    (condition_4, "Missing or incorrect Cost for 5 Year Deactivation Window"),
    (condition_5, "Past-due Planned Deactivation from previous fiscal"),
    (condition_6, "Overlapping or Duplicated Planned Deactivation")
]
...
output_name_pattern = f'{output_folder}/DeactivationLiability_Conformance Summary_YYYY_MM_DD.xlsx'
...
with pd.ExcelWriter(output_path) as writer:
    ...
    for sheet_name in selected_df['BUSINESS_AREA_CODE'].unique().tolist() + ['Province']: # Iterate through each regional + province sheet
        ...
        for row_idx, (_, row) in enumerate(region_df.iterrows(), start=1):
            ...
            if (
                ((row['RCLS_ACCOUNTING_TYPE'] == '1 Sale = 5 yrs') or (row['RCLS_ACCOUNTING_TYPE'] == 'S.Term = 10 yrs')) and 
                (row['DEAC_METHOD_TYPE'] == '')
            ):
            ...
            if (
                ((row['RCLS_ACCOUNTING_TYPE'] == '1 Sale = 5 yrs') or (row['RCLS_ACCOUNTING_TYPE'] == 'S.Term = 10 yrs')) and
                ((row['EFFECTIVE_PLANNED_COST'] == '') or (row['EFFECTIVE_PLANNED_COST'] == 0) or (row['EFFECTIVE_PLANNED_COST'] == 1))
            ):
                worksheet.write(row_idx, column_to_index['RCLS_ACCOUNTING_TYPE'], row['RCLS_ACCOUNTING_TYPE'], yellow_format) 
                worksheet.write(row_idx, column_to_index['EFFECTIVE_PLANNED_COST'], row['EFFECTIVE_PLANNED_COST'], red_format)

            if (
                ((row['RCLS_ACCOUNTING_TYPE'] == '1 Sale = 5 yrs') or (row['RCLS_ACCOUNTING_TYPE'] == 'S.Term = 10 yrs')) and
                (row['DEAC_PLANNED_DATE'] == '')
            ):
                worksheet.write(row_idx, column_to_index['RCLS_ACCOUNTING_TYPE'], row['RCLS_ACCOUNTING_TYPE'], yellow_format)
                worksheet.write(row_idx, column_to_index['DEAC_PLANNED_DATE'], row['DEAC_PLANNED_DATE'], red_format)    

            if (
                ((row['PLANNED_FISCAL_YEAR'] != '') and (row['PLANNED_FISCAL_YEAR'] < (current_fiscal + 5))) and
                ((row['EFFECTIVE_PLANNED_COST'] == '') or (row['EFFECTIVE_PLANNED_COST'] == 0) or (row['EFFECTIVE_PLANNED_COST'] == 1))
            ):
                worksheet.write(row_idx, column_to_index['PLANNED_FISCAL_YEAR'], row['PLANNED_FISCAL_YEAR'], yellow_format)
                worksheet.write(row_idx, column_to_index['EFFECTIVE_PLANNED_COST'], row['EFFECTIVE_PLANNED_COST'], red_format)  

            if ((row['PLANNED_FISCAL_YEAR'] != '') and (row['PLANNED_FISCAL_YEAR'] < current_fiscal)):
                worksheet.write(row_idx, column_to_index['PLANNED_FISCAL_YEAR'], row['PLANNED_FISCAL_YEAR'], red_format) 

            if (row['OVER_LAPPING'] == 'Y'):
                worksheet.write(row_idx, column_to_index['OVER_LAPPING'], row['OVER_LAPPING'], yellow_format)
                worksheet.write(row_idx, column_to_index['POC'], row['POC'], red_format) 
                worksheet.write(row_idx, column_to_index['POT'], row['POT'], red_format)
