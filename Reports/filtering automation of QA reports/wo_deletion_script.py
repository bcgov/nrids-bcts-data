...
##### 
# Filter ‘WOFF_STATUS’ ‘Done’ and delete
# Filter ‘DEL_STATUS’ ‘Done’ and delete
#####
df_1 = df_1[~((df_1['WOFF_STATUS'] == 'D') | (df_1['DEL_STATUS'] == 'D'))]

# Condition 1: If needs to reallocate to properly named licence WO bucket (“!Write off” format)
condition_1 = (
    (df_1['WOFF_STATUS'] == 'P') &
    ~(df_1['LICENCE_ID'].str.contains(r'(?i)(?=.*write)(?=.*off)|WO', na=False))
)

# Condition 2: If DVC ‘Done’ blocks can NEVER be deleted
condition_2 = (
    (df_1['DEL_STATUS'] == 'P') &
    (df_1['DVC_STATUS'] == 'D')
)

# Condition 3: If Past due planned deletion
condition_3 = (
    (df_1['DEL_STATUS'] == 'P') &
    (df_1['DEL_FISCAL'] < current_fiscal)
)

# Condition 4: If Planned deletion with no planned date
condition_4 = (
    (df_1['DEL_STATUS'] == 'P') &
    (df_1['DEL_Status_Date'].isnull())
)

# Condition 5: If Planned deletion for future fiscal
condition_5 = (
    (df_1['DEL_STATUS'] == 'P') &
    (df_1['DEL_FISCAL'] > current_fiscal)
)

# Condition 6: If Missing DVS ‘Done’ with $ spent on layout; planned for deletion
condition_6 = (
    (df_1['DEL_STATUS'] == 'P') &
    (
        (df_1['DVS_STATUS'] == 'N') |
        (df_1['DVS_STATUS'] == 'P') |
        (df_1['DVS_STATUS'].isnull())
        ) &
    (df_1['Layout_Amount'] > 1000)
)

# Condition 7: If Past due planned write-off
condition_7 = (
    (df_1['WOFF_STATUS'] == 'P') &
    (df_1['WOFF_FISCAL'] < current_fiscal) 
)

# Condition 8: If Planned write-off with no planned date
condition_8 = (
    (df_1['WOFF_STATUS'] == 'P') &
    (df_1['WOFF_Status_Date'].isnull())
)

# Condition 9: If Planned write-off for future fiscal
condition_9 = (
    (df_1['WOFF_STATUS'] == 'P') &
    (df_1['WOFF_FISCAL'] > current_fiscal)
)

# Condition 10: If Planned write-off and deletion
condition_10 = (
    (df_1['WOFF_STATUS'] == 'P') &
    (df_1['DEL_STATUS'] == 'P')
)

# Condition 11: If Planned write-off without volume
condition_11 = (
    (df_1['WOFF_STATUS'] == 'P') &
    ((df_1['BLAL_CRUISE_M3_VOL'] == 0) | (df_1['BLAL_CRUISE_M3_VOL'].isnull())) &
    ((df_1['RW_VOL'] == 0) | (df_1['RW_VOL'].isnull()))
)

# Condition 12: Planned write-off with no spatial
condition_12 = (
    (df_1['WOFF_STATUS'] == 'P') &
    (df_1['SPATIAL_LINKED'] == 'N')
)

# Condition 13: Missing DVS ‘Done’ with $ spent on layout; planned for WO
condition_13 = (
    (df_1['WOFF_STATUS'] == 'P') &
    (df_1['DVS_STATUS'] != 'D') &
    (df_1['DVC_STATUS'] != 'D') &
...
# Condition 14: Block State X indicates WO 'Considered' with no ‘Planned or Done’ activity
condition_14 = (
    (df_1['DEL_STATUS'] != 'P') &
    (df_1['WOFF_STATUS'] != 'P') &
    (df_1['CUTB_BLOCK_STATE'] == 'X')
)

conditions = [
    (condition_1, "Reallocate to properly named licence WO bucket (“!Write off” format)"),
    (condition_2, "DVC ‘Done’ blocks can NEVER be deleted"),
    (condition_3, "Past due planned deletion"),
    (condition_4, "Planned deletion with no planned date"),
    (condition_5, "Planned deletion for future fiscal"),
    (condition_6, "Missing DVS ‘Done’ with $ spent on layout; planned for deletion"),
    (condition_7, "Past due planned write-off"),
    (condition_8, "Planned write-off with no planned date"),
    (condition_9, "Planned write-off for future fiscal"),
    (condition_10, "Planned write-off and deletion"),
    (condition_11, "Planned write-off without volume"),
    (condition_12, "Planned write-off with no spatial"),
    (condition_13, "Missing DVS ‘Done’ with $ spent on layout; planned for WO"),
    (condition_14, "Block State X indicates WO 'Considered' with no ‘Planned or Done’ activity")
]
...
output_name_pattern = f"{output_folder}/Planned_WO'sorDeletions_Conformance_Summary_YYYY_MM_DD.xlsx"
    ...
    for sheet_name in selected_df['BUSINESS_AREA_CODE'].unique().tolist() + ['Province']: # Iterate through each regional + province sheet
        ...
        for row_idx, (_, row) in enumerate(region_df.iterrows(), start=1): # Iterate through each row of region_df and apply condition highlighting format
            ...
            if (
                ...
                worksheet.write(row_idx, column_to_index['DEL_STATUS'], row['DEL_STATUS'], yellow_format)
                worksheet.write(row_idx, column_to_index['DEL_FISCAL'], row['DEL_FISCAL'], red_format)  
            ...
            if (
                ...
                worksheet.write(row_idx, column_to_index['DEL_STATUS'], row['DEL_STATUS'], yellow_format)
                worksheet.write(row_idx, column_to_index['DEL_Status_Date'], row['DEL_Status_Date'], red_date_format)
            ...
            if (
                ...
                worksheet.write(row_idx, column_to_index['WOFF_STATUS'], row['WOFF_STATUS'], yellow_format)
                worksheet.write(row_idx, column_to_index['WOFF_FISCAL'], row['WOFF_FISCAL'], red_format)

            if (
                (new_columns[7] in columns_with_Y) and
                (row[new_columns[7]] == 'Y')
                ):
                worksheet.write(row_idx, column_to_index['WOFF_STATUS'], row['WPFF_STATUS'], yellow_format)
                worksheet.write(row_idx, column_to_index['WOFF_Status_Date'], row['WOFF_Status_Date'], red_date_format)
            ...
            if (
                (new_columns[13] in columns_with_Y) and
                (row[new_columns[13]] == 'Y')
                ):
                worksheet.write(row_idx, column_to_index['WOFF_STATUS'], row['WOFF_STATUS'], yellow_format)
                worksheet.write(row_idx, column_to_index['DEL_STATUS'], row['DEL_STATUS'], yellow_format) 
                worksheet.write(row_idx, column_to_index['CUTB_BLOCK_STATE'], row['CUTB_BLOCK_STATE'], red_format) 
