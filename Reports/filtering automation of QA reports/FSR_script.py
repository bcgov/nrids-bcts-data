...
try:
    ...
    file_pattern = os.path.join(script_dir, '../data', 'QA_BCTS_FSR_Inventory_*_*.xlsx')
    ...
    if not matching_files:
        raise FileNotFoundError(
            ...
            'QA_BCTS_FSR_Inventory_YYYY_MM_DD.xlsx'
            """
        )
    ...
    df = pd.read_excel(file_path, sheet_name='qQA_BCTS_FSR_Inventory')

except FileNotFoundError as e:
    print(f"Error: {e}")

# Change the datatype of the MARK_SEQ_NBR to Int64 (which was a float before)
df['ROAD_SEQ_NBR'] = df['ROAD_SEQ_NBR'].astype('Int64')
...
# Condition 1: If Future Deactivation Completed Date
condition_1 = (
    (df_1['URI'].notnull()) &
    (df_1['RDST_STEWARD_NAME'] == 'BCTS') &
    (df_1['RCLS_ACCOUNTING_TYPE'].notnull()) &
    (df_1['DEAC_END_DATE'] > date_value)
)

# Condition 2: If Future Construction Completed Date
condition_2 = (
    (df_1['URI'].notnull()) &
    (df_1['RDST_STEWARD_NAME'] == 'BCTS') &
    (df_1['RCLS_ACCOUNTING_TYPE'].notnull()) &
    (df_1['RCOM_COMPLETION_DATE'] > date_value)
)

# Condition 3: If Missing construction date for non-deactivated BCTS FSR
condition_3 = (
    (df_1['URI'].notnull()) &
    (df_1['RDST_STEWARD_NAME'] == 'BCTS') &
    (df_1['RCLS_ACCOUNTING_TYPE'].notnull()) &
    (df_1['RCOM_COMPLETION_DATE'].isnull()) &
    (df_1['DEAC_END_DATE'].isnull())
)

# Condition 4: If Missing risk rating for un-deactivated constructed BCTS FSR
condition_4 = (
    (df_1['URI'].notnull()) &
    (df_1['RDST_STEWARD_NAME'] == 'BCTS') &
    (df_1['RCLS_ACCOUNTING_TYPE'].notnull()) &
    ((df_1['RRRA_RISK_RATING'].isnull()) | (df_1['RRRA_RISK_RATING'] == 'To Determine')) &
    (df_1['DEAC_END_DATE'].isnull())
)

# Condition 5: If Missing Steward for road with accounting type (means constructed) and URI
condition_5 = (
    (df_1['RCLS_ACCOUNTING_TYPE'].notnull()) &
    (df_1['URI'].notnull()) &
    (df_1['RDST_STEWARD_NAME'].isnull())
)

# Condition 6: If Missing URI with accounting type (means constructed) and BCTS Steward
condition_6 = (
    (df_1['RDST_STEWARD_NAME'] == 'BCTS') &
    (df_1['RCLS_ACCOUNTING_TYPE'].notnull()) &
    (df_1['URI'].isnull())
...
# Condition 7: If Missing acct type (means constructed) for constructed and non-deactivated BCTS Steward with URI
condition_7 =(
    (df_1['RDST_STEWARD_NAME'] == 'BCTS') &
    (df_1['RCLS_ACCOUNTING_TYPE'].isnull()) &
    (df_1['URI'].notnull()) &
    (df_1['RCOM_COMPLETION_DATE'].notnull()) &
    (df_1['DEAC_END_DATE'].isnull())
)

# Condition 8: Still BCTS Steward without completed deactivation
condition_8 = (
    (df_1['RDST_STEWARD_NAME'] == 'former BCTS') &
    (df_1['URI'].notnull()) &
    (df_1['RCLS_ACCOUNTING_TYPE'].notnull()) &
    (df_1['DEAC_END_DATE'].isnull()) 
)

# Condition 9: If Still BCTS Steward with insufficiently deactivated FSR
condition_9 = (
    (df_1['RDST_STEWARD_NAME'] == 'former BCTS') &
    (df_1['URI'].notnull()) &
    (df_1['RCLS_ACCOUNTING_TYPE'].notnull()) &
    (
        (df_1['DEAC_LEVEL_TYPE'] == 'Seasonal') |
        (df_1['DEAC_LEVEL_TYPE'] == 'Temporary') |
        (df_1['DEAC_LEVEL_TYPE'] == 'Semi-Permanent') 
        )
)

# Condition 10: If no new Steward with completed transfer out 
condition_10 = (
    (df_1['RDST_STEWARD_NAME'] == 'BCTS') &
    (df_1['URI'].notnull()) &
    (df_1['RCLS_ACCOUNTING_TYPE'].notnull()) &
    (df_1['DEAC_METHOD_TYPE'] == 'TRANSFER OUT') &
    (df_1['DEAC_END_DATE'].notnull())
)

# Condition 11: If Still BCTS Steward and needs accounting type without completed deactivation or transferred out 
condition_11 = (
    (df_1['RDST_STEWARD_NAME'] == 'former BCTS') &
    (df_1['URI'].notnull()) &
    (df_1['RCLS_ACCOUNTING_TYPE'].isnull()) &
...
# Condition 12: If Must be BCTS Steward without completed deactivation or transferred out
condition_12 = (
    (
        (df_1['RDST_STEWARD_NAME'] != 'BCTS') &
        (df_1['RDST_STEWARD_NAME'] != 'former BCTS') &
        (df_1['RDST_STEWARD_NAME'].notnull())
        ) &
        (df_1['URI'].notnull()) &
        (df_1['RCLS_ACCOUNTING_TYPE'].notnull()) &
        (df_1['DEAC_END_DATE'].isnull()) &
...
# Condition 13: If Maintenance level type only and doesn’t qualify for designated Method Type
condition_13 = (
    (df_1['RDST_STEWARD_NAME'] == 'BCTS') &
    (df_1['RCLS_ACCOUNTING_TYPE'].notnull()) &
    (df_1['URI'].notnull()) &
    (df_1['DEAC_END_DATE'].notnull()) &
    (
        (df_1['DEAC_LEVEL_TYPE'] != 'Permanent') &
        (df_1['DEAC_LEVEL_TYPE'] != 'PERMANENT') &
        (df_1['DEAC_LEVEL_TYPE'].notnull())
        ) &
    (
        (df_1['DEAC_METHOD_TYPE'] == 'DEACT') |
        (df_1['DEAC_METHOD_TYPE'] == 'REHAB') |
        (df_1['DEAC_METHOD_TYPE'] == 'TRANSFER OUT')
        )
)
...
conditions = [
    (condition_1, "Future Deactivation Completed Date"),
    (condition_2, "Future Construction Completed Date"),
    (condition_3, "Missing construction date for non-deactivated BCTS FSR"),
    (condition_4, "Missing risk rating for un-deactivated constructed BCTS FSR"),
    (condition_5, "Missing Steward for road with accounting type (means constructed) and URI"),
    (condition_6, "Missing URI with accounting type (means constructed) and BCTS Steward"),
    (condition_7, "Missing acct type (means constructed) for constructed and non-deactivated BCTS Steward with URI"),
    (condition_8, "Still BCTS Steward without completed deactivation"),
    (condition_9, "Still BCTS Steward with insufficiently deactivated FSR"),
    (condition_10, "Must be assigned new Steward with completed transfer out"),
    (condition_11, "Still BCTS Steward and needs accounting type without completed deactivation or transferred out"),
    (condition_12, "Must be BCTS Steward without completed deactivation or transferred out"),
    (condition_13, "Maintenance level type only and doesn't qualify for designated Method Type; confirm level and adjust data accordingly")
]
...
new_columns = [con[1] for con in conditions]
...
output_name_pattern = f'{output_folder}/BCTS_FSRInventory_Conformance_Summary_YYYY_MM_DD.xlsx'
...
with pd.ExcelWriter(output_path) as writer:
    ...
    for sheet_name in selected_df['BUSINESS_AREA_CODE'].unique().tolist() + ['Province']: # Iterate through each regional + province sheet
        ...
        for row_idx, (_, row) in enumerate(region_df.iterrows(), start=1): # Iterate through each row of region_df and apply condition highlighting format
            if (
                (new_columns[0] in columns_with_Y) and
                (row[new_columns[0]] == 'Y')
                ):
                worksheet.write(row_idx, column_to_index['URI'], row['URI'], yellow_format)
                worksheet.write(row_idx, column_to_index['RDST_STEWARD_NAME'], row['RDST_STEWARD_NAME'], yellow_format) 
                worksheet.write(row_idx, column_to_index['RCLS_ACCOUNTING_TYPE'], row['RCLS_ACCOUNTING_TYPE'], yellow_format)  
                worksheet.write(row_idx, column_to_index['DEAC_END_DATE'], row['DEAC_END_DATE'], red_date_format)  

            if (
                (new_columns[1] in columns_with_Y) and
                (row[new_columns[1]] == 'Y')
                ):
                worksheet.write(row_idx, column_to_index['URI'], row['URI'], yellow_format)
                worksheet.write(row_idx, column_to_index['RDST_STEWARD_NAME'], row['RDST_STEWARD_NAME'], yellow_format) 
                worksheet.write(row_idx, column_to_index['RCLS_ACCOUNTING_TYPE'], row['RCLS_ACCOUNTING_TYPE'], yellow_format)  
                worksheet.write(row_idx, column_to_index['RCOM_COMPLETION_DATE'], row['RCOM_COMPLETION_DATE'], red_date_format)  

            if (
                (new_columns[2] in columns_with_Y) and
                (row[new_columns[2]] == 'Y')
                ):
                worksheet.write(row_idx, column_to_index['URI'], row['URI'], yellow_format)
                worksheet.write(row_idx, column_to_index['RDST_STEWARD_NAME'], row['RDST_STEWARD_NAME'], yellow_format) 
                worksheet.write(row_idx, column_to_index['RCLS_ACCOUNTING_TYPE'], row['RCLS_ACCOUNTING_TYPE'], yellow_format)  
                worksheet.write(row_idx, column_to_index['DEAC_END_DATE'], row['DEAC_END_DATE'], yellow_date_format) 
                worksheet.write(row_idx, column_to_index['RCOM_COMPLETION_DATE'], row['RCOM_COMPLETION_DATE'], red_format)  

            if (
                (new_columns[3] in columns_with_Y) and
                (row[new_columns[3]] == 'Y')
                ):
                worksheet.write(row_idx, column_to_index['URI'], row['URI'], yellow_format)
                worksheet.write(row_idx, column_to_index['RDST_STEWARD_NAME'], row['RDST_STEWARD_NAME'], yellow_format) 
                worksheet.write(row_idx, column_to_index['RCLS_ACCOUNTING_TYPE'], row['RCLS_ACCOUNTING_TYPE'], yellow_format) 
                worksheet.write(row_idx, column_to_index['DEAC_END_DATE'], row['DEAC_END_DATE'], yellow_date_format)  
                worksheet.write(row_idx, column_to_index['RRRA_RISK_RATING'], row['RRRA_RISK_RATING'], red_format)  

            if (
                (new_columns[4] in columns_with_Y) and
                (row[new_columns[4]] == 'Y')
                ):
                worksheet.write(row_idx, column_to_index['RCLS_ACCOUNTING_TYPE'], row['RCLS_ACCOUNTING_TYPE'], yellow_format)
                worksheet.write(row_idx, column_to_index['URI'], row['URI'], yellow_format)
                worksheet.write(row_idx, column_to_index['RDST_STEWARD_NAME'], row['RDST_STEWARD_NAME'], red_format)

            if (
                (new_columns[5] in columns_with_Y) and
                (row[new_columns[5]] == 'Y')
                ):
                worksheet.write(row_idx, column_to_index['RDST_STEWARD_NAME'], row['RDST_STEWARD_NAME'], yellow_format)
                worksheet.write(row_idx, column_to_index['RCLS_ACCOUNTING_TYPE'], row['RCLS_ACCOUNTING_TYPE'], yellow_format)
                worksheet.write(row_idx, column_to_index['URI'], row['URI'], red_format)

            if (
                (new_columns[6] in columns_with_Y) and
                (row[new_columns[6]] == 'Y')
                ):
                worksheet.write(row_idx, column_to_index['RDST_STEWARD_NAME'], row['RDST_STEWARD_NAME'], yellow_format)
                worksheet.write(row_idx, column_to_index['URI'], row['URI'], yellow_format)
                worksheet.write(row_idx, column_to_index['RCOM_COMPLETION_DATE'], row['RCOM_COMPLETION_DATE'], yellow_date_format) 
                worksheet.write(row_idx, column_to_index['DEAC_END_DATE'], row['DEAC_END_DATE'], yellow_date_format)  
                worksheet.write(row_idx, column_to_index['RCLS_ACCOUNTING_TYPE'], row['RCLS_ACCOUNTING_TYPE'], red_format)

            if (
                (new_columns[7] in columns_with_Y) and
                (row[new_columns[7]] == 'Y')
                ):
                worksheet.write(row_idx, column_to_index['URI'], row['URI'], yellow_format)
                worksheet.write(row_idx, column_to_index['RCLS_ACCOUNTING_TYPE'], row['RCLS_ACCOUNTING_TYPE'], yellow_format)
            ...
            if (
                (new_columns[8] in columns_with_Y) and
                (row[new_columns[8]] == 'Y')
                ):
                worksheet.write(row_idx, column_to_index['RDST_STEWARD_NAME'], row['RDST_STEWARD_NAME'], yellow_format)
                worksheet.write(row_idx, column_to_index['URI'], row['URI'], yellow_format)
                worksheet.write(row_idx, column_to_index['RCLS_ACCOUNTING_TYPE'], row['RCLS_ACCOUNTING_TYPE'], yellow_format)
                worksheet.write(row_idx, column_to_index['DEAC_LEVEL_TYPE'], row['DEAC_LEVEL_TYPE'], red_format)
            
            if (
                (new_columns[9] in columns_with_Y) and
                (row[new_columns[9]] == 'Y')
                ):
                worksheet.write(row_idx, column_to_index['URI'], row['URI'], yellow_format)
                worksheet.write(row_idx, column_to_index['RCLS_ACCOUNTING_TYPE'], row['RCLS_ACCOUNTING_TYPE'], yellow_format)
                worksheet.write(row_idx, column_to_index['DEAC_END_DATE'], row['DEAC_END_DATE'], yellow_date_format) 
                worksheet.write(row_idx, column_to_index['DEAC_METHOD_TYPE'], row['DEAC_METHOD_TYPE'], yellow_format)
                worksheet.write(row_idx, column_to_index['RDST_STEWARD_NAME'], row['RDST_STEWARD_NAME'], red_format)

            if (
                (new_columns[10] in columns_with_Y) and
                (row[new_columns[10]] == 'Y')
                ):
                worksheet.write(row_idx, column_to_index['URI'], row['URI'], yellow_format)
                worksheet.write(row_idx, column_to_index['DEAC_END_DATE'], row['DEAC_END_DATE'], yellow_date_format) 
                worksheet.write(row_idx, column_to_index['RCOM_COMPLETION_DATE'], row['RCOM_COMPLETION_DATE'], yellow_date_format)
            ...
            if (
                (new_columns[11] in columns_with_Y) and
                (row[new_columns[11]] == 'Y')
                ):
                worksheet.write(row_idx, column_to_index['URI'], row['URI'], yellow_format)
                worksheet.write(row_idx, column_to_index['DEAC_END_DATE'], row['DEAC_END_DATE'], yellow_date_format) 
                worksheet.write(row_idx, column_to_index['RCOM_COMPLETION_DATE'], row['RCOM_COMPLETION_DATE'], yellow_date_format)
            ...
            if (
                (new_columns[12] in columns_with_Y) and
                (row[new_columns[12]] == 'Y')
                ):
                worksheet.write(row_idx, column_to_index['RDST_STEWARD_NAME'], row['RDST_STEWARD_NAME'], yellow_format)
                worksheet.write(row_idx, column_to_index['URI'], row['URI'], yellow_format)
                worksheet.write(row_idx, column_to_index['RCLS_ACCOUNTING_TYPE'], row['RCLS_ACCOUNTING_TYPE'], yellow_format)
                worksheet.write(row_idx, column_to_index['DEAC_END_DATE'], row['DEAC_END_DATE'], yellow_date_format) 
                worksheet.write(row_idx, column_to_index['DEAC_METHOD_TYPE'], row['DEAC_METHOD_TYPE'], red_format)
                worksheet.write(row_idx, column_to_index['DEAC_LEVEL_TYPE'], row['DEAC_LEVEL_TYPE'], red_format)
