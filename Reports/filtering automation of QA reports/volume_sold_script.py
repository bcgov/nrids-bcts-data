...
try:
    ...
    file_pattern = os.path.join(script_dir, '../data', 'QA_VolumeLicenceIssued_*_*.xlsx')
    ...
    if not matching_files:
        raise FileNotFoundError(
            ...
            'QA_VolumeLicenceIssued_*_*.xlsx'
            """
        )
    ...
    df = pd.read_excel(file_path, sheet_name='qQA_VolumeLicenceIssued')
...
# Condition 1: If Volume variance between LRM Total Volume and Admin Sale Volume
condition_1 = (
    df_1['LRM_Admin_Var_Over_100_m3'] == 'Y'
)

# Condition 2: If Volume variance between FTA and LRM Total Volume
condition_2 = (
    df_1['LRM_FTA_Var_Over_100_m3'] == 'Y'
)

# Condition 3: If Volume variance between LRM Total Volume and ECAS Total Volume
condition_3 = (
    df_1['LRM_ECAS_Var_Over_100_m3'] == 'Y'
)

# Condition 4: If Volume variance between Admin and FTA Volume
condition_4 = (
    df_1['FTA_Admin_Var_Over_100_m3'] == 'Y'
)

# Make a list for all the conditions and their corresponding column names
conditions = [
    (condition_1, "Volume variance between LRM Total Volume and Admin Sale Volume"),
    (condition_2, "Volume variance between FTA and LRM Total Volume"),
    (condition_3, "Volume variance between LRM Total Volume and ECAS Total Volume"),
    (condition_4, "Volume variance between Admin and FTA Volume")
]
...
con_column_names = [
    ...
    'LRM_TOTAL_VOLUME',
    'LRM_Admin_Variance',
    'LRM_Admin_Var_Over_100_m3',
    'LRM_ECAS_Variance',
    'LRM_ECAS_Var_Over_100_m3',
    'LRM_FTA_Variance',
    'LRM_FTA_Var_Over_100_m3',
    'FTA_Admin_Variance',
    'FTA_Admin_Var_Over_100_m3'
]
...
output_name_pattern = f'{output_folder}/VolumeLicenceIssued_Conformance_Summary_YYYY_MM_DD.xlsx'
...
with pd.ExcelWriter(output_path) as writer:
    ...
    for sheet_name in selected_df['BUSINESS_AREA_CODE'].unique().tolist() + ['Province']: # Iterate through each regional + province sheet
        ...
        for row_idx, (_, row) in enumerate(region_df.iterrows(), start=1):
            ...
            if (
                row['LRM_Admin_Var_Over_100_m3'] == 'Y'
            ):
                worksheet.write(row_idx, column_to_index['LRM_Admin_Var_Over_100_m3'], row['LRM_Admin_Var_Over_100_m3'], yellow_format)
                worksheet.write(row_idx, column_to_index['LRM_Admin_Variance'], row['LRM_Admin_Variance'], yellow_format)
                worksheet.write(row_idx, column_to_index['Admin_Sale_volume'], row['Admin_Sale_volume'], red_format)
                worksheet.write(row_idx, column_to_index['LRM_TOTAL_VOLUME'], row['LRM_TOTAL_VOLUME'], red_format)

            if (
                row['LRM_FTA_Var_Over_100_m3'] == 'Y'
            ):
                worksheet.write(row_idx, column_to_index['LRM_FTA_Var_Over_100_m3'], row['LRM_FTA_Var_Over_100_m3'], yellow_format)
                worksheet.write(row_idx, column_to_index['LRM_FTA_Variance'], row['LRM_FTA_Variance'], yellow_format)
                worksheet.write(row_idx, column_to_index['FTA_Volume'], row['FTA_Volume'], red_format)
                worksheet.write(row_idx, column_to_index['LRM_TOTAL_VOLUME'], row['LRM_TOTAL_VOLUME'], red_format)

            if (
                row['LRM_ECAS_Var_Over_100_m3'] == 'Y'
            ):
                worksheet.write(row_idx, column_to_index['LRM_ECAS_Var_Over_100_m3'], row['LRM_ECAS_Var_Over_100_m3'], yellow_format)
                worksheet.write(row_idx, column_to_index['LRM_ECAS_Variance'], row['LRM_ECAS_Variance'], yellow_format)
                worksheet.write(row_idx, column_to_index['ECAS_TOTAL_VOLUME'], row['ECAS_TOTAL_VOLUME'], red_format)
                worksheet.write(row_idx, column_to_index['LRM_TOTAL_VOLUME'], row['LRM_TOTAL_VOLUME'], red_format)
                
            if (
                row['FTA_Admin_Var_Over_100_m3'] == 'Y'
            ):
                worksheet.write(row_idx, column_to_index['FTA_Admin_Var_Over_100_m3'], row['FTA_Admin_Var_Over_100_m3'], yellow_format)
                worksheet.write(row_idx, column_to_index['FTA_Admin_Variance'], row['FTA_Admin_Variance'], yellow_format)
                worksheet.write(row_idx, column_to_index['FTA_Volume'], row['FTA_Volume'], red_format)
                worksheet.write(row_idx, column_to_index['Admin_Sale_volume'], row['Admin_Sale_volume'], red_format)
